...
/**
 * A controller for handling sale transactions with atomicity guarantees.
 */
module.exports = {
   async createSaleTransaction(ctx) {
      try {
         ...
         //Use Strapi's built-in transaction API to ensure atomicity
         const result = await strapi.db.transaction(async (trx) => {
            ...
            for (const productItem of data.products) {
               try {
                  ...
                  if (updatedStock < 0) {
                     throw new Error(`Insufficient stock for product ID ${productItem.product}`);
                  }

                  //Check if we would go negative stock
                  if (updatedStock < 0) {
                     throw new Error(`Insufficient stock for product ID ${productItem.product}`);
                  }
                  ...
                  console.error(`Error updating stock for product ID ${productItem.product}:`, err);
